<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>ghar.com</title>
    <link
      rel="stylesheet"
      href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css"
      integrity="sha512-SnH5WK+bZxgPHs44uWIX+LLJAJ9/2PkPKZ5QiAj6Ta86w+fsb2TkcmfRyVX3pBnMFcV7oQPJkl9QevSCWr3W6A=="
      crossorigin="anonymous"
      referrerpolicy="no-referrer"
    />
    <link rel="stylesheet" href="gharstyle.css" />
    <link rel="website icon" type="png" href="/GHAR_1/gharimg/ghar.png" />
  </head>
  <body>
    <div class="body">
      <header class="nav-header">
        <nav>
          <div class="nav-bar">
            <span class="nav-img">
              <img
                src="/GHAR_1/gharimg/ghar.png"
                alt=" Ghar logo"
                width="100px"
              />
            </span>

            <ul class="navbar-menu">
              <li>
                <a class="get-ideas" href="/GHAR_1/indexnavbar_pages/getideas.php"
                  >Get Ideas
                </a>
              </li>
              <li>
                <a class="find-pro" href="\GHAR_1\professionals fetch\fetch_professional.php"
                  >Find Professionals</a
                >
              </li>
              <li>
                <a class="shop-product" href="\GHAR_1\indexnavbar_pages\Shopnow.php">Shop Products</a>
              </li>
            </ul>

            <div class="nav-search">
              <input placeholder="Search" class="search-input" />

              <div class="search-icon">
                <i class="fa-solid fa-magnifying-glass"></i>
              </div>
            </div>

            <div class="nav-third">
              <span class="cart-icon">
                <a class="cart-move" href="indexnavbar_pages\emptycart.php">

                  <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
                  <svg
                    width="35px"
                    viewBox="0 0 14 14"
                    role="img"
                    focusable="false"
                    aria-hidden="true"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <g transform="translate(.13043484 .04347832) scale(.28986)">
                      <g fill="#333333">
                        <path
                          d="M18 32c-1.1 0-2-.9-2-2V20h-4v10c0 3.3 2.7 6 6 6h19v-4H18z"
                        />

                        <path
                          d="M12.8 10l-.4-1.3C11.8 7.1 10.3 6 8.6 6H5v4h3.6l5.5 16.6c.3.8 1 1.4 1.9 1.4h19c.8 0 1.5-.5 1.8-1.2L44.4 10H12.8z"
                        />
                      </g>

                      <circle cx="5" cy="8" r="2" fill="#00695c" />

                      <g fill="#333333">
                        <circle cx="34" cy="39" r="3" />

                        <circle cx="19" cy="39" r="3" />
                      </g>

                      <g fill="#333333">
                        <circle cx="34" cy="39" r="1" />

                        <circle cx="19" cy="39" r="1" />
                      </g>
                    </g>
                  </svg>
                  Cart
                </a>
              </span>
            </div>

            <div class="nav-forth">
              <a href="authentication/Signin.php">
                <span class="signin-icon">
                  <svg
                    width="35px"
                    fill="#333"
                    viewBox="0 0 32 32"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M16 15.503A5.041 5.041 0 1 0 16 5.42a5.041 5.041 0 0 0 0 10.083zm0 2.215c-6.703 0-11 3.699-11 5.5v3.363h22v-3.363c0-2.178-4.068-5.5-11-5.5z"
                    />
                  </svg>
                  Sign In
                </span>
              </a>
            </div>

            <div class="nav-second">
              <a href="\GHAR_1\authentication\profsignin.php">
                <span class="joinpro-icon">

                  <!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.0//EN" "http://www.w3.org/TR/2001/REC-SVG-20010904/DTD/svg10.dtd">
                  <svg
                    version="1.0"
                    id="Layer_1"
                    xmlns="http://www.w3.org/2000/svg"
                    xmlns:xlink="http://www.w3.org/1999/xlink"
                    width="30px"
                    viewBox="0 0 64 64"
                    enable-background="new 0 0 64 64"
                    xml:space="preserve"
                  >
                    <g>
                      <path
                        fill="#333333"
                        d="M18,4c0-1.104,0.896-2,2-2h24c1.104,0,2,0.896,2,2v9h-2V5c0-0.553-0.447-1-1-1H21c-0.553,0-1,0.447-1,1v8
		h-2V4z"
                      />
                      <path
                        fill="#333333"
                        d="M60,13H48V4c0-2.211-1.789-4-4-4H20c-2.211,0-4,1.789-4,4v9H4c-2.211,0-4,1.789-4,4v43c0,2.211,1.789,4,4,4
		h56c2.211,0,4-1.789,4-4V17C64,14.789,62.211,13,60,13z M18,4c0-1.104,0.896-2,2-2h24c1.104,0,2,0.896,2,2v9h-2V5
		c0-0.553-0.447-1-1-1H21c-0.553,0-1,0.447-1,1v8h-2V4z M42,6v7H22V6H42z M62,60c0,1.104-0.896,2-2,2H4c-1.104,0-2-0.896-2-2V42h10
		v5c0,0.553,0.447,1,1,1h6c0.553,0,1-0.447,1-1v-5h24v5c0,0.553,0.447,1,1,1h6c0.553,0,1-0.447,1-1v-5h10V60z M14,46v-4v-2v-4h4v4v2
		v4H14z M46,46v-4v-2v-4h4v4v2v4H46z M62,40H52v-5c0-0.553-0.447-1-1-1h-6c-0.553,0-1,0.447-1,1v5H20v-5c0-0.553-0.447-1-1-1h-6
		c-0.553,0-1,0.447-1,1v5H2V17c0-1.104,0.896-2,2-2h56c1.104,0,2,0.896,2,2V40z"
                      />
                      <g>
                        <rect
                          x="46"
                          y="36"
                          fill="#333333"
                          width="4"
                          height="10"
                        />
                        <rect
                          x="14"
                          y="36"
                          fill="#333333"
                          width="4"
                          height="10"
                        />
                      </g>
                      <g>
                        <path
                          fill="#333333"
                          d="M60,62c1.104,0,2-0.896,2-2V42H52v5c0,0.553-0.447,1-1,1h-6c-0.553,0-1-0.447-1-1v-5H20v5
			c0,0.553-0.447,1-1,1h-6c-0.553,0-1-0.447-1-1v-5H2v18c0,1.104,0.896,2,2,2H60z"
                        />
                        <path
                          fill="#333333"
                          d="M60,15H4c-1.104,0-2,0.896-2,2v23h10v-5c0-0.553,0.447-1,1-1h6c0.553,0,1,0.447,1,1v5h24v-5
			c0-0.553,0.447-1,1-1h6c0.553,0,1,0.447,1,1v5h10V17C62,15.896,61.104,15,60,15z"
                        />
                      </g>
                      <path
                        opacity="0.2"
                        d="M60,62c1.104,0,2-0.896,2-2V42H52v5c0,0.553-0.447,1-1,1h-6c-0.553,0-1-0.447-1-1v-5H20v5
		c0,0.553-0.447,1-1,1h-6c-0.553,0-1-0.447-1-1v-5H2v18c0,1.104,0.896,2,2,2H60z"
                      />
                    </g>
                  </svg>
                  Join as a Pro
                </span>
              </a>
            </div>
          </div>
        </nav>
      </header>

      <main>
        <div class="main">
          <div class="container">
            <section class="home-container">
              <div>
                <div class="home-content">
                  <h1 class="head">
                    <header>The Best Place To Build Your Ghar</header>
                  </h1>
                  <p class="head-text">
                    "Ghar" is a place where relationships deepen, traditions
                    thrive, and bonds strengthen—a haven that resonates with the
                    heartbeat of family and love.
                  </p>
                  <div class="main-container">
                    <div class="main-box">
                      <input
                        type="text"
                        placeholder="What service do you need?"
                        class="main-text"
                      />
                    </div>
                    <div class="main-zip">
                      <div class="main-input">
                        <input
                          type="text"
                          placeholder="Zip Code"
                          class="zip-text"
                        />
                        <div class="location-icon">
                          <i class="fa-solid fa-location-dot"></i>
                        </div>
                      </div>
                      <div class="getstart">
                        <button class="button-text" type="button">
                          Get Started
                        </button>
                      </div>
                    </div>
                  </div>
                  <section class="pro-tags">
                    <div class="down-text">Hire a pro:</div>
                    <div class="home-info">
                      <a href=""><span>Contractors</span></a>
                      <a href=""><span>Interior Designer</span></a>
                      <a href=""><span>Architects</span></a>
                      <a href=""><span>See More</span></a>
                    </div>
                  </section>
                </div>
              </div>
            </section>
            <aside class="pro-section-container">
              <div>
                <header class="pro-head">Join Ghar</header>
                <p class="pro-paragraph">
                  Get the all-in-one tools for building homes
                </p>
                <div class="email-section">
                  <button class="button-labels" type="button"><a href="/GHAR_1/registration/usersreg.php">
                    Join for Free
                  </a></button>
                  <div class="disclaimer">
                    <p>
                      By signing up, signing in or continuing, I agree to Ghar's
                      Terms of Use and Privacy Policy.
                    </p>
                  </div>
                </div>
              </div>
            </aside>
          </div>

          <section>
            <div class="landing-container">
              <div class="landing-content">Here's what you can do on Ghar</div>
              <div class="box">
                <div class="landing-design">
                  <a href="#">
                    <div class="design-logo">
                      <img src="/GHAR_1/gharimg/discoverideas.png" />
                      <div class="design-text">Discover Design</div>
                    </div>
                  </a>
                </div>

                <div class="landing-product">
                  <a href="/GHAR/furniture.html">
                    <div class="product-logo">
                      <img src="/GHAR_1/gharimg/shopproduct.png" />
                      <div class="product-text">Shop Products</div>
                    </div>
                  </a>
                </div>

                <div class="landing-pros">
                  <a href="/WEB/GHAR/professionals.html">
                    <div class="pros-logo">
                      <img src="/GHAR_1/gharimg/browsepro.png" />
                      <div class="pros-text">Find Pros</div>
                    </div>
                  </a>
                </div>

                <div class="landing-suggest">
                  <a href="#">
                    <div class="suggest-logo">
                      <img src="/GHAR_1/gharimg/suggestpros.png" />
                      <div class="suggest-text">Suggest Pros</div>
                    </div>
                  </a>
                </div>
              </div>
            </div>
          </section>

          <section>
            <div class="login-container">
              <div class="log-info">
                <header class="login-head">
                  Make Your Dream Home a Reality
                </header>
                <p class="login-pg">
                  Find inspiration, products and the pros to make it happen -
                  all in one place
                </p>
              </div>

              <aside class="google-section">
                <div class="google-login">
                  <div>
                    <button class="google-button" id="googleButton" type="submit">
                      <div class="google-icon">
                        <svg
                          width="25px"
                          viewBox="0 0 32 32"
                          data-name="Layer 1"
                          id="Layer_1"
                          xmlns="http://www.w3.org/2000/svg"
                        >
                          <path
                            d="M23.75,16A7.7446,7.7446,0,0,1,8.7177,18.6259L4.2849,22.1721A13.244,13.244,0,0,0,29.25,16"
                            fill="#00ac47"
                          />
                          <path
                            d="M23.75,16a7.7387,7.7387,0,0,1-3.2516,6.2987l4.3824,3.5059A13.2042,13.2042,0,0,0,29.25,16"
                            fill="#4285f4"
                          />
                          <path
                            d="M8.25,16a7.698,7.698,0,0,1,.4677-2.6259L4.2849,9.8279a13.177,13.177,0,0,0,0,12.3442l4.4328-3.5462A7.698,7.698,0,0,1,8.25,16Z"
                            fill="#ffba00"
                          />
                          <polygon
                            fill="#2ab2db"
                            points="8.718 13.374 8.718 13.374 8.718 13.374 8.718 13.374"
                          />
                          <path
                            d="M16,8.25a7.699,7.699,0,0,1,4.558,1.4958l4.06-3.7893A13.2152,13.2152,0,0,0,4.2849,9.8279l4.4328,3.5462A7.756,7.756,0,0,1,16,8.25Z"
                            fill="#ea4435"
                          />
                          <polygon
                            fill="#2ab2db"
                            points="8.718 18.626 8.718 18.626 8.718 18.626 8.718 18.626"
                          />
                          <path
                            d="M29.25,15v1L27,19.5H16.5V14H28.25A1,1,0,0,1,29.25,15Z"
                            fill="#4285f4"
                          />
                        </svg>
                      </div>
                      <span class="google-continue">Continue with Google</span>
                    </button>
                    <script src="https://accounts.google.com/gsi/client" async defer></script>
                    <script src="/WEB/GHAR/googlebutton.js" defer></script>
                  </div>
                  <div class="line-or">
                    <hr />
                    Or
                    <hr />
                  </div>
                  <div class="email-signup">
                    <input
                      class="email-text"
                      type="email"
                      placeholder="Email"
                    />
                  </div>
                  <div>
                    <button class="signup" type="submit">
                      <span>Sign Up with Email</span>
                    </button>
                  </div>
                  <p class="google-parag">
                    By signing up, signing in or continuing, I agree to Ghar's
                    Terms of Use and Privacy
                  </p>
                </div>
              </aside>
            </div>
          </section>

          <section class="section">
            <div class="shop-card">
              <div class="card-head">
                <h2>Shop by Department</h2>
                <div class="card-all">
                  <span>See All</span>
                </div>
              </div>
              <div class="card-body">
                <div class="front-img">
                  <img
                    src="/GHAR_1/gharimg/kitchen.jpg"
                    width="154px"
                    height="154px"
                  />
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a href="/WEB/GHAR/">
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/Marble.jpg"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Marble</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/Faucets.jpg"
                            width="153px"
                            height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Faucets</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a href="/WEB/GHAR/Table.css">
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/kitchen&Dinning.jpg"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Kitchen & Dinning</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a href="/GHAR_1/Chair.html">
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/Kitchen Sink.jpg"
                            width="153px"
                            height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Kitchen Sink</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/outdoor.webp"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>OutDoor</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/lighting.webp"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Lighting</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a href="/WEB/GHAR/">
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/Chimney.jpg" width="153px" height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Chimney</p>
                      </div>
                    </a>
                  </div>
                </div>

              </div>
            </div>
          </section>

          <section class="section">
            <div class="shop-card">
              <div class="card-head">
                <h2>Browse by Professionals</h2>
                <div class="card-all">
                  <span>See All</span>
                </div>
              </div>
              <div class="card-body">
                <div class="front-img">
                  <img
                    src="/GHAR_1/gharimg/design.jpg"
                    width="154px"
                    height="154px"
                  />
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/designbuildfirm.webp"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Design-Build Firms</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/generalcontractor.webp"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>General Contractors</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/Homebuilders.webp"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Home Builders</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/Interiordesigner.webp"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Interior Designers</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/kitchenremodel.webp"  width="153px" height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Bathroom Remodel</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/stones.webp"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Stones & Pavers</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/homeremodelling.jpg"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Home Remodel</p>
                      </div>
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </section>

          <section class="section">
            <div class="shop-card">
              <div class="card-head">
                <h2>Shop by Raw Materials</h2>
                <div class="card-all">
                  <span>See All</span>
                </div>
              </div>
              <div class="card-body">
                <div class="front-img">
                  <img
                    src="/GHAR_1/gharimg/rawmaterials.jpg"
                    width="154px"
                    height="154px"
                  />
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/Brick.jpg"
                            width="153px"
                            height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Bricks</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/Cement.jpeg"
                            width="153px"
                            height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Cement</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/Rods.png"
                            width="153px"
                            height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Rods</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/Pipes.png"
                            width="153px"
                            height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Pipes</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/Aggregates.jpg"
                            width="153px"
                            height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Aggregates</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img src="/GHAR_1/gharimg/glass.jpg" width="153px" height="95px"></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Glass</p>
                      </div>
                    </a>
                  </div>
                </div>

                <div class="furniture-container">
                  <div class="card-furniture">
                    <a>
                      <div class="product-img">
                        <picture
                          ><img
                            src="/GHAR_1/gharimg/sand.jpg"
                            width="153px"
                            height="95px"
                        /></picture>
                      </div>
                      <div class="furniture-text">
                        <p>Sand</p>
                      </div>
                    </a>
                  </div>
                </div>
              </div>
            </div>
          </section>
        </div>
      </main>
     <!--                                               FOOTER SECTION                                                                 -->
     <footer class="footer">
        <div class="footer-content">
          <div class="footer-logo">
            <img src="/GHAR_1/gharimg/ghar.png" alt="Ghar logo" width="100px" />
            <a href="/WEB/GHAR/indexghar.html" class="ghar-ho"><p>Ghar</p></a>
          </div>
          <div class="footer-links">
            <div>
              <h3>Explore</h3>
              <a target="_blank" href="/GHAR_1/product_fetch/Sofa_fetch.php">Sofa</a>
              <a target="_blank" href="/GHAR_1/product_fetch/Table_fetch.php">Table</a>
              <a target="_blank" href="/GHAR_1/product_fetch/Chair_fetch.php">Chairs</a>
              <a target="_blank" href="/GHAR_1/product_fetch/Cement_fetch.php">Cement</a>
            </div>
            <div>
              <h3>Professionals</h3>
              <a target="_blank" href="/GHAR_1/professionals fetch/fetch_professional.php">Find Professionals</a>
            </div>
            <div>
              <h3>Company</h3>
              <a target="_blank" href="\GHAR_1\Footer_pages\aboutghar.php">About Ghar</a>
              <a  target=”_blank”  href="\GHAR_1\Footer_pages\career.php">Careers</a>
              <a target="_blank" href="\GHAR_1\Footer_pages\Contact.php">Contact Us</a>
            </div>
            <div>
              <h3>Help</h3>
              <a target="_blank" href="\GHAR_1\Footer_pages\customersupport.php">Customer Support</a>
              <a target="_blank" href="\GHAR_1\Footer_pages\faq.php">FAQ</a>
              <a target="_blank" href="\GHAR_1\Footer_pages\termsofuse.php">Terms of Use</a>
              <a target="_blank" href="\GHAR_1\Footer_pages\privacypolicy.php">Privacy Policy</a>
            </div>
          </div>
          <div class="footer-social">
            <a href="https://www.facebook.com/" class="facebook">
              <svg
                width="25px"
                height="25px"
                viewBox="0 0 266.895 266.895"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M252.164 266.895c8.134 0 14.729-6.596 14.729-14.73V14.73c0-8.137-6.596-14.73-14.729-14.73H14.73C6.593 0 0 6.594 0 14.73v237.434c0 8.135 6.593 14.73 14.73 14.73h237.434z"
                  fill="#4267B2"
                />
                <path
                  d="M184.152 266.895V163.539h34.692l5.194-40.28h-39.887V97.542c0-11.662 3.238-19.609 19.962-19.609l21.329-.01V41.897c-3.689-.49-16.351-1.587-31.08-1.587-30.753 0-51.807 18.771-51.807 53.244v29.705h-34.781v40.28h34.781v103.355h41.597z"
                  fill="#ffffff"
                />
              </svg>
            </a>
            <a href="#" class="twitter">
              <svg
                fill="black"
                class="svgIcon"
                xmlns="http://www.w3.org/2000/svg"
                width="25px"
                height="25px"
                viewBox="0 0 564 564"
              >
                <path
                  d="M389.2 48h70.6L305.6 224.2 487 464H345L233.7 318.6 106.5 464H35.8L200.7 275.5 26.8 48H172.4L272.9 180.9 389.2 48zM364.4 421.8h39.1L151.1 88h-42L364.4 421.8z"
                ></path>
              </svg>
            </a>
            <a href="https://www.instagram.com/" class="instagram">
              <svg
                width="25px"
                height="25px"
                viewBox="0 0 3364.7 3364.7"
                xmlns="http://www.w3.org/2000/svg"
              >
                <defs>
                  <radialGradient
                    id="0"
                    cx="217.76"
                    cy="3290.99"
                    r="4271.92"
                    gradientUnits="userSpaceOnUse"
                  >
                    <stop offset=".09" stop-color="#fa8f21" />
                    <stop offset=".78" stop-color="#d82d7e" />
                  </radialGradient>
                  <radialGradient
                    id="1"
                    cx="2330.61"
                    cy="3182.95"
                    r="3759.33"
                    gradientUnits="userSpaceOnUse"
                  >
                    <stop offset=".64" stop-color="#8c3aaa" stop-opacity="0" />
                    <stop offset="1" stop-color="#8c3aaa" />
                  </radialGradient>
                </defs>
                <path
                  d="M853.2,3352.8c-200.1-9.1-308.8-42.4-381.1-70.6-95.8-37.3-164.1-81.7-236-153.5S119.7,2988.6,82.6,2892.8c-28.2-72.3-61.5-181-70.6-381.1C2,2295.4,0,2230.5,0,1682.5s2.2-612.8,11.9-829.3C21,653.1,54.5,544.6,82.5,472.1,119.8,376.3,164.3,308,236,236c71.8-71.8,140.1-116.4,236-153.5C544.3,54.3,653,21,853.1,11.9,1069.5,2,1134.5,0,1682.3,0c548,0,612.8,2.2,829.3,11.9,200.1,9.1,308.6,42.6,381.1,70.6,95.8,37.1,164.1,81.7,236,153.5s116.2,140.2,153.5,236c28.2,72.3,61.5,181,70.6,381.1,9.9,216.5,11.9,281.3,11.9,829.3,0,547.8-2,612.8-11.9,829.3-9.1,200.1-42.6,308.8-70.6,381.1-37.3,95.8-81.7,164.1-153.5,235.9s-140.2,116.2-236,153.5c-72.3,28.2-181,61.5-381.1,70.6-216.3,9.9-281.3,11.9-829.3,11.9-547.8,0-612.8-1.9-829.1-11.9"
                  fill="url(#0)"
                />
                <path
                  d="M853.2,3352.8c-200.1-9.1-308.8-42.4-381.1-70.6-95.8-37.3-164.1-81.7-236-153.5S119.7,2988.6,82.6,2892.8c-28.2-72.3-61.5-181-70.6-381.1C2,2295.4,0,2230.5,0,1682.5s2.2-612.8,11.9-829.3C21,653.1,54.5,544.6,82.5,472.1,119.8,376.3,164.3,308,236,236c71.8-71.8,140.1-116.4,236-153.5C544.3,54.3,653,21,853.1,11.9,1069.5,2,1134.5,0,1682.3,0c548,0,612.8,2.2,829.3,11.9,200.1,9.1,308.6,42.6,381.1,70.6,95.8,37.1,164.1,81.7,236,153.5s116.2,140.2,153.5,236c28.2,72.3,61.5,181,70.6,381.1,9.9,216.5,11.9,281.3,11.9,829.3,0,547.8-2,612.8-11.9,829.3-9.1,200.1-42.6,308.8-70.6,381.1-37.3,95.8-81.7,164.1-153.5,235.9s-140.2,116.2-236,153.5c-72.3,28.2-181,61.5-381.1,70.6-216.3,9.9-281.3,11.9-829.3,11.9-547.8,0-612.8-1.9-829.1-11.9"
                  fill="url(#1)"
                />
                <path
                  d="M1269.25,1689.52c0-230.11,186.49-416.7,416.6-416.7s416.7,186.59,416.7,416.7-186.59,416.7-416.7,416.7-416.6-186.59-416.6-416.7m-225.26,0c0,354.5,287.36,641.86,641.86,641.86s641.86-287.36,641.86-641.86-287.36-641.86-641.86-641.86S1044,1335,1044,1689.52m1159.13-667.31a150,150,0,1,0,150.06-149.94h-0.06a150.07,150.07,0,0,0-150,149.94M1180.85,2707c-121.87-5.55-188.11-25.85-232.13-43-58.36-22.72-100-49.78-143.78-93.5s-70.88-85.32-93.5-143.68c-17.16-44-37.46-110.26-43-232.13-6.06-131.76-7.27-171.34-7.27-505.15s1.31-373.28,7.27-505.15c5.55-121.87,26-188,43-232.13,22.72-58.36,49.78-100,93.5-143.78s85.32-70.88,143.78-93.5c44-17.16,110.26-37.46,232.13-43,131.76-6.06,171.34-7.27,505-7.27S2059.13,666,2191,672c121.87,5.55,188,26,232.13,43,58.36,22.62,100,49.78,143.78,93.5s70.78,85.42,93.5,143.78c17.16,44,37.46,110.26,43,232.13,6.06,131.87,7.27,171.34,7.27,505.15s-1.21,373.28-7.27,505.15c-5.55,121.87-25.95,188.11-43,232.13-22.72,58.36-49.78,100-93.5,143.68s-85.42,70.78-143.78,93.5c-44,17.16-110.26,37.46-232.13,43-131.76,6.06-171.34,7.27-505.15,7.27s-373.28-1.21-505-7.27M1170.5,447.09c-133.07,6.06-224,27.16-303.41,58.06-82.19,31.91-151.86,74.72-221.43,144.18S533.39,788.47,501.48,870.76c-30.9,79.46-52,170.34-58.06,303.41-6.16,133.28-7.57,175.89-7.57,515.35s1.41,382.07,7.57,515.35c6.06,133.08,27.16,223.95,58.06,303.41,31.91,82.19,74.62,152,144.18,221.43s139.14,112.18,221.43,144.18c79.56,30.9,170.34,52,303.41,58.06,133.35,6.06,175.89,7.57,515.35,7.57s382.07-1.41,515.35-7.57c133.08-6.06,223.95-27.16,303.41-58.06,82.19-32,151.86-74.72,221.43-144.18s112.18-139.24,144.18-221.43c30.9-79.46,52.1-170.34,58.06-303.41,6.06-133.38,7.47-175.89,7.47-515.35s-1.41-382.07-7.47-515.35c-6.06-133.08-27.16-224-58.06-303.41-32-82.19-74.72-151.86-144.18-221.43S2586.8,537.06,2504.71,505.15c-79.56-30.9-170.44-52.1-303.41-58.06C2068,441,2025.41,439.52,1686,439.52s-382.1,1.41-515.45,7.57"
                  fill="#ffffff"
                /></svg></a>
          </div>
          <p class="terms">
            By signing up, you agree to our
            <a  target=”_blank”  href="\GHAR_1\Footer_pages\Termsofservice.php" class="link"
              >Terms of Service</a
            >.
          </p>
          <p>&copy; 2024 GHAR. All rights reserved.</p>
        </div>
      </footer>
    </div>
  </body>
</html>